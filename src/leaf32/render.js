...
function LeafRenderLarge(_size, leaf, leafLeftGrid, leafTopGrid, renderCfg) {
  let {imageData, viewport, scale: pixelsPerCell, colors} = renderCfg
    ...
    , columnStart = floor(boundedLeftGrid  - leafLeftGrid) | 0
    , rowStart    = floor(boundedTopGrid   - leafTopGrid)  | 0
    // ...and the indexes of the first columns/rows to NOT be drawn
    , columnEnd   = ceil(boundedRightGrid  - leafLeftGrid) | 0
    , rowEnd      = ceil(boundedBottomGrid - leafTopGrid)  | 0
...
function LeafRenderSmall(_size, leaf, leafLeftGrid, leafTopGrid, renderCfg) {
  let {imageData, viewport, scale: pixelsPerCell, colors} = renderCfg
    , {data, width: imageWidth, height: imageHeight} = imageData
    , {
        left:   viewportLeftGrid,
    ...
    // By rounding "out" we get...
    // ...the indexes of the first columns/rows to be drawn
    , columnStart = ceil(boundedLeftGrid  - leafLeftGrid) | 0
    , rowStart    = ceil(boundedTopGrid   - leafTopGrid)  | 0
    // ...and the indexes of the first columns/rows to NOT be drawn
    , columnEnd   = floor(boundedRightGrid  - leafLeftGrid) | 0
    , rowEnd      = floor(boundedBottomGrid - leafTopGrid)  | 0
  imageWidth |= 0; imageHeight |= 0;
  for (let rowIndex = rowStart; rowIndex < rowEnd; rowIndex++) {
    let row = leaf[rowIndex] | 0
      , y = (rowIndex * pixelsPerCell) + leafTopImage | 0
